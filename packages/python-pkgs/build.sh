	...
	patch_src() {
		...
		case $PYTHON_PKG in
			...
			torch )
				...
				;;
			platformdirs )
				# ImportError: cannot import name '__version_info__' from 'platformdirs.version' (/data/data/com.termux/files/usr/lib/python3.10/dist-packages/platformdirs/version.py)
				# file generated by setuptools_scm
				sed -i -e 's|, __version_info__||' -e '/"__version_info__",/d' src/platformdirs/__init__.py
				;;
			python-pptx )
				# AttributeError: module 'collections' has no attribute 'abc'
				sed -i -E 's|^import collections$|import collections.abc|' pptx/compat/__init__.py
		esac
	}
